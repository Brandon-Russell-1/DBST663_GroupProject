...
FOR EACH ROW
   ...
   BEGIN
    ...
    END;
    /
/*5 Queries for Explain Plan*/

/* Explain #1 */
/* This query is selecting the providers who are prescribing a specific medication */
SET TIMING ON
EXPLAIN PLAN FOR 
SELECT PRESCRIPTIONS.PROVIDER_ID, PROVIDER.FIRST_NAME, PROVIDER.LAST_NAME FROM  PRESCRIPTIONS
JOIN PROVIDER
ON PRESCRIPTIONS.PROVIDER_ID=PROVIDER.PROVIDER_ID
WHERE PRESCRIPTIONS.MEDICATION_ID='2';
